export default function Btn(props) {
  const estilos = {
    ...
    "buscar-vagas": "rounded-xl bg-laranja- text-white hover:bg-laranja-escuro hover:scale-110 transition duration-300",
    "ver-vagas": "border-2 rounded-xl px-2.5 py-2.5 bg-transparent",
    "redefinir-busca": "rounded-xl px-3.5 py-2.5 bg-lime-700",
  ...
  const class_parent = props.className

  const classeDoBotao = estilos[layout] || estilos["classe-padrao"];

  return <button className={`${classeDoBotao} ${class_parent} no-underline`}>{text}</button>;
}
